...
import com.lucilu.marvelpedia.base.data.common.store.ReactivePersistence.Disk;
...
public class ReactiveStore<Key, Value> {
    ...
    @NonNull
    private final Memory<Key, Value> cache;

    @NonNull
    private final Option<Disk<Key, Value>> disk;

    public ReactiveStore(@NonNull final Memory<Key, Value> cache) {
        this.cache = cache;
        disk = none();
    }

    public ReactiveStore(@NonNull final Memory<Key, Value> cache, @NonNull final Disk<Key, Value> disk) {
        this.cache = cache;
